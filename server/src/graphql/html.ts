import { TMemoryStatus, TMemoryAverage } from "../types"
 ...
 */
export function memoryLeakageWarningHTML({clientTimestamp,jsHeapSizeLimit,totalJSHeapSize, usedJSHeapSize, location, growth, limit}: TMemoryStatus & {growth: number, limit: number}): string {
 ...
 */
export function dailyReportHTML({clientTimestamp,jsHeapSizeLimit,totalJSHeapSize, usedJSHeapSize, location, growth, limit, average}: TMemoryStatus & {growth: number, limit: number, average: TMemoryAverage}): string {
  return `<h2>Memory Documentation</h2>
  <h3>Daily Report<h3>
...
/**
 * Function that returns a string containing HTML for an email with the given memory useage.
 */
export function memoryLogRequestHTML(memoryStatus: TMemoryStatus): string {
  const {
    clientTimestamp,
    jsHeapSizeLimit,
    totalJSHeapSize,
